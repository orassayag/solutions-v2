...
const createPhotosFromJsons = (photos) => {
    // Validate parameters.
    ...
    // Loop on all photos data.
    for (let i = 0; i < photos.length; i++) {
        // Validate photo basic data.
        const photoData = photos[i];
        const validateUserErrorMessage = validatePhoto(photoData, i);
        if (validateUserErrorMessage.errorMessage) {
            winston.error(validateUserErrorMessage.errorMessage);
            continue;
        }

        // Push the photo into the array.
        photosList.push(new Photo({
...
module.exports = {
    ...
    createPhotosFromJsons: createPhotosFromJsons
};